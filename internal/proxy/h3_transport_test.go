...
func TestBuildTransports_H3UDPBuffers(t *testing.T) {
	t.Run("no h3 transport for cleartext backend", func(t *testing.T) {
		ts := buildTransports("http", config.TransportConfig{}, 30*time.Second, 100, 90*time.Second, false, false)
		assert.Nil(t, ts.h3, "h3 transport must be nil for cleartext (http) backends")
	})

	t.Run("h3 transport created for HTTPS backend without buffer config", func(t *testing.T) {
		ts := buildTransports("https", config.TransportConfig{}, 30*time.Second, 100, 90*time.Second, true, false)
		require.NotNil(t, ts.h3, "h3 transport must be non-nil for HTTPS backends")
		assert.Nil(t, ts.h3.Dial, "Dial should be nil when no buffer sizes are configured")
	})

	t.Run("h3 transport gets custom Dial when receive buffer is set", func(t *testing.T) {
		ts := buildTransports("https", config.TransportConfig{
			H3UDPReceiveBufferSize: 7500000,
		}, 30*time.Second, 100, 90*time.Second, true, false)
		require.NotNil(t, ts.h3)
		assert.NotNil(t, ts.h3.Dial, "Dial must be set when H3UDPReceiveBufferSize > 0")
	})
	...
	t.Run("h3 transport gets custom Dial when send buffer is set", func(t *testing.T) {
		...
		require.NotNil(t, ts.h3)
		assert.NotNil(t, ts.h3.Dial, "Dial must be set when H3UDPSendBufferSize > 0")
	})
	...
	t.Run("h3 transport gets custom Dial when both buffers are set", func(t *testing.T) {
		...
		require.NotNil(t, ts.h3)
		assert.NotNil(t, ts.h3.Dial, "Dial must be set when both buffer sizes > 0")
	})
	...
	t.Run("h3 transport Dial is nil when buffer sizes are zero", func(t *testing.T) {
		...
		require.NotNil(t, ts.h3)
		assert.Nil(t, ts.h3.Dial, "Dial should be nil when buffer sizes are 0")
	})
	...
	t.Run("h3 transport preserves TLS config regardless of buffer settings", func(t *testing.T) {
		...
		require.NotNil(t, ts.h3)
		require.NotNil(t, ts.h3.TLSClientConfig, "TLSClientConfig must always be set")
		assert.True(t, ts.h3.TLSClientConfig.InsecureSkipVerify,
	...
	t.Run("h3 transport respects TLS insecure flag when buffers configured", func(t *testing.T) {
		...
		require.NotNil(t, ts.h3)
		assert.False(t, ts.h3.TLSClientConfig.InsecureSkipVerify,
